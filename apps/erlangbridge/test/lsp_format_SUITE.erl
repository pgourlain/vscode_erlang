...
all() -> [
    ...
    result_range_is_hardcoded_to_the_whole_document
].
...
%% CHARACTERIZATION (see task 5.3): textDocument_formatting/2 always
%% returns one TextEdit spanning the fixed range {0,0}-{999999,255},
%% regardless of the document's real length - not the document's actual
%% end position. Any file, however short, gets this same oversized range.
result_range_is_hardcoded_to_the_whole_document(Config) ->
    AppDir = ?config(data_dir, Config),
