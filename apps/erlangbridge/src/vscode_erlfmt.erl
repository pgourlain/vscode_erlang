...
which_pragma_node(Node) ->
    {PreComments, _, PostComments} = vscode_erlfmt_format:comments_with_pre_dot(Node),
...
insert_pragma_node(Node) ->
    PreComments = vscode_erlfmt_scan:get_anno(pre_comments, Node, []),
    NewPreComments =
        case PreComments of
            [] ->
                Loc = vscode_erlfmt_scan:get_anno(location, Node),
    ...
    vscode_erlfmt_scan:put_anno(pre_comments, NewPreComments, Node).
...
remove_pragma_node(Node0) ->
    {PreComments0, _, PostComments0} = vscode_erlfmt_format:comments_with_pre_dot(Node0),
    PreComments = remove_pragma_comment_blocks(PreComments0),
    PostComments = remove_pragma_comment_blocks(PostComments0),
    Node = vscode_erlfmt_scan:put_anno(pre_comments, PreComments, Node0),
    vscode_erlfmt_scan:put_anno(post_comments, PostComments, Node).
...
-spec read_nodes(file:name_all()) ->
    {ok, [vscode_erlfmt_parse:abstract_form()], [error_info()]} | {error, error_info()}.
read_nodes(FileName) ->
    try
...
file_read_nodes(FileName, Pragma) ->
    read_file(FileName, fun(File) ->
        read_nodes(vscode_erlfmt_scan:io_node(File), FileName, Pragma)
    end).
...
read_nodes_string(FileName, String, Pragma) ->
    read_nodes(vscode_erlfmt_scan:string_node(String), FileName, Pragma).
...
read_nodes({ok, Tokens, Comments, Cont}, FileName, PragmaFlag, [], Warnings0, TextAcc) ->
    ...
    case {which_pragma_node(Node), Node} of
        {_, {shebang, _, _}} ->
            {LastString, _Anno} = vscode_erlfmt_scan:last_node_string(Cont),
            read_nodes(
                vscode_erlfmt_scan:continue(Cont),
                FileName,
                PragmaFlag,
                [Node],
                Warnings,
...
skip_nodes(Cont, FileName, TextAcc) ->
    {LastString, _Anno} = vscode_erlfmt_scan:last_node_string(Cont),
    case vscode_erlfmt_scan:read_rest(Cont) of
        {ok, Rest} ->
...
read_nodes_loop({ok, Tokens, Comments, Cont}, FileName, Acc, Warnings0) ->
    {Node, Warnings} = parse_node(Tokens, Comments, FileName, Cont, Warnings0),
    read_nodes_loop(vscode_erlfmt_scan:continue(Cont), FileName, [Node | Acc], Warnings);
...
parse_node([{shebang, Meta, String}], Comments, _FileName, _Cont, Warnings) ->
    {{shebang, vscode_erlfmt_recomment:put_post_comments(Meta, Comments), String}, Warnings};
parse_node([Token | _] = Tokens, Comments, FileName, Cont, Warnings) ->
    {PreComments, _} = vscode_erlfmt_recomment:take_comments(vscode_erlfmt_scan:get_line(Token), Comments),
    case lists:any(fun contains_ignore_comment/1, PreComments) of
        false ->
            case vscode_erlfmt_parse:parse_node(Tokens) of
                {ok, Node} ->
                    {vscode_erlfmt_recomment:recomment(Node, Comments), Warnings};
                {error, {ErrLoc, Mod, Reason}} ->
...
node_string(Cont) ->
    {String, Anno} = vscode_erlfmt_scan:last_node_string_trimmed(Cont),
    {raw_string, Anno, String}.

-spec format_nodes([vscode_erlfmt_parse:abstract_form()], pos_integer()) -> [unicode:chardata()].
...
-spec format_node(vscode_erlfmt_parse:abstract_form(), pos_integer()) -> unicode:chardata().
format_node({raw_string, _Anno, String}, _PrintWidth) ->
    String;
format_node(Node, PrintWidth) ->
    Doc = vscode_erlfmt_format:to_algebra(Node),
    vscode_erlfmt_algebra:format(Doc, PrintWidth).
...
try_location(Node, _) when is_tuple(Node) -> vscode_erlfmt_scan:get_anno(location, Node);
try_location([Node | _], _) when is_tuple(Node) -> vscode_erlfmt_scan:get_anno(location, Node);
try_location(_, Node) when is_tuple(Node) -> vscode_erlfmt_scan:get_anno(location, Node);
try_location(_, [Node | _]) when is_tuple(Node) -> vscode_erlfmt_scan:get_anno(location, Node);
try_location(_, _) -> 0.
...
get_location(Node) ->
    vscode_erlfmt_scan:get_anno(location, Node).

get_end_location(Node) ->
    vscode_erlfmt_scan:get_anno(end_location, Node).
