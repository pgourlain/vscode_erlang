...
-module(vscode_erlfmt_recomment).
...
-spec recomment(vscode_erlfmt_parse:abstract_node(), [vscode_erlfmt_scan:comment()]) ->
    vscode_erlfmt_parse:abstract_node().
...
put_pre_dot_comments(NodeOrMeta, Comments) ->
    Existing = vscode_erlfmt_scan:get_anno(pre_dot_comments, NodeOrMeta, []),
    vscode_erlfmt_scan:merge_anno(
        #{
...
put_post_comments(NodeOrMeta, Comments) ->
    Loc = vscode_erlfmt_scan:get_anno(end_location, NodeOrMeta),
    CommentLoc = vscode_erlfmt_scan:get_anno(end_location, lists:last(Comments)),
    Existing = vscode_erlfmt_scan:get_anno(post_comments, NodeOrMeta, []),
    vscode_erlfmt_scan:merge_anno(
        #{
...
put_pre_comments(NodeOrMeta, Comments) ->
    Loc = vscode_erlfmt_scan:get_anno(location, NodeOrMeta),
    CommentLoc = vscode_erlfmt_scan:get_anno(location, hd(Comments)),
    Existing = vscode_erlfmt_scan:get_anno(pre_comments, NodeOrMeta, []),
    vscode_erlfmt_scan:merge_anno(
        #{
