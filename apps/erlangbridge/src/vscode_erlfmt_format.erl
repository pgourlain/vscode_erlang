...
-spec to_algebra(vscode_erlfmt_parse:abstract_form()) -> vscode_erlfmt_algebra:doc().
...
do_expr_to_algebra({string, Meta, _Value}) ->
    string_to_algebra(vscode_erlfmt_scan:get_anno(text, Meta));
do_expr_to_algebra({char, #{text := "$ "}, $\s}) ->
    <<"$\\s">>;
do_expr_to_algebra({Atomic, Meta, _Value}) when ?IS_ATOMIC(Atomic) ->
    string(vscode_erlfmt_scan:get_anno(text, Meta));
...
concat_to_algebra([Value1, Value2 | _] = Values) ->
    ...
    case has_break_between(Value1, Value2) of
        true ->
            concat(force_breaks(), group(fold_doc(fun vscode_erlfmt_algebra:line/2, ValuesD)));
        false ->
            group(fold_doc(fun vscode_erlfmt_algebra:break/2, ValuesD))
    end.
...
unary_needs_space({op, Meta, _, _}, _) ->
    not vscode_erlfmt_scan:get_anno(parens, Meta, false);
...
binary_op_to_algebra(Op, Meta0, Left0, Right0) ->
    ...
    Meta = vscode_erlfmt_scan:delete_annos([parens, pre_comments, post_comments], Meta0),
...
rewrite_assoc(Op, MetaABC0, A, {op, MetaBC0, Op, B0, C} = BC) ->
    case vscode_erlfmt_scan:get_anno(parens, MetaBC0, false) of
        true ->
            {op, MetaABC0, Op, A, BC};
        _ ->
            {Pre, Post} = comments(MetaBC0),
            MetaBC = vscode_erlfmt_scan:delete_annos([pre_comments, post_comments], MetaBC0),
            B = vscode_erlfmt_recomment:put_pre_comments(B0, Pre),
            AB = rewrite_assoc(Op, update_meta_location(MetaBC, A, B), A, B),
            MetaABC = vscode_erlfmt_recomment:put_post_comments(MetaABC0, Post),
...
update_meta_location(Meta, First, Last) ->
    Meta#{
        location := vscode_erlfmt_scan:get_anno(location, First),
        end_location := vscode_erlfmt_scan:get_anno(end_location, Last)
    }.
...
binary_operand_to_algebra(Op, {op, Meta, Op, Left, Right}, Indent) ->
    ...
    case vscode_erlfmt_scan:get_anno(parens, Meta, false) of
...
-spec last_fits_fun(last_normal | last_fits, [vscode_erlfmt_parse:abstract_form()]) ->
    fun((vscode_erlfmt_algebra:doc(), disabled | enabled) -> vscode_erlfmt_algebra:doc()).
last_fits_fun(last_fits, Values) when Values =/= [] ->
    case is_next_break_fits(lists:last(Values)) of
        true -> fun vscode_erlfmt_algebra:next_break_fits/2;
...
-spec has_opening_line_break(vscode_erlfmt_scan:anno(), [vscode_erlfmt_parse:abstract_form()]) -> boolean().
...
-spec break_fun(break_behaviour()) -> vscode_erlfmt_algebra:append_fun().
break_fun(flex_break) -> fun vscode_erlfmt_algebra:flex_break/2;
break_fun(_) -> fun vscode_erlfmt_algebra:break/2.

-type value_doc_pair() :: {vscode_erlfmt_parse:abstract_form(), vscode_erlfmt_algebra:doc()}.
...
-spec join_inner_values(vscode_erlfmt_algebra:append_fun(), [value_doc_pair()]) -> vscode_erlfmt_algebra:doc().
...
fa_groups([{op, Meta0, '/', {atom, _, HeadFunction}, _} = Head0 | Tail]) ->
    ...
    case lists:splitwith(Splitter, Tail) of
        ...
        {Group, Rest} ->
            Head = vscode_erlfmt_scan:delete_annos([pre_comments, post_comments], Head0),
            Meta = vscode_erlfmt_scan:range_anno(Meta0, lists:last(Group)),
...
record_name_to_algebra(Meta, Name) ->
    ...
    case vscode_erlfmt_scan:get_anno(macro_record, Meta, false) of
...
try_to_algebra(Body, OfClauses, CatchClauses, After) ->
    Clauses =
        [try_of_block(Body, OfClauses)] ++
            ...
            [<<"end">>],
    concat(force_breaks(), (group(fold_doc(fun vscode_erlfmt_algebra:line/2, Clauses)))).
...
is_multiline(Node) ->
    vscode_erlfmt_scan:get_inner_line(Node) =/= vscode_erlfmt_scan:get_inner_end_line(Node).

has_break_between(Left, [Right | _]) ->
    has_break_between(Left, Right);
has_break_between(Left, Right) ->
    vscode_erlfmt_scan:get_end_line(Left) < vscode_erlfmt_scan:get_line(Right).

has_empty_line_between(Left, Right) ->
    vscode_erlfmt_scan:get_end_line(Left) + 1 < vscode_erlfmt_scan:get_line(Right).

has_inner_break(Outer, Inner) ->
    vscode_erlfmt_scan:get_inner_line(Outer) < vscode_erlfmt_scan:get_line(Inner).
...
has_no_comments_or_parens(Meta) ->
    ...
    Pre =:= [] andalso Post =:= [] andalso not vscode_erlfmt_scan:get_anno(parens, Meta, false).
...
combine_pre_comments(Comments, Meta, Doc) ->
    case
        vscode_erlfmt_scan:get_end_line(lists:last(Comments)) + 1 <
            vscode_erlfmt_scan:get_inner_line(Meta)
    of
...
combine_post_comments([Comment | _] = Comments, Meta, Doc) ->
    case vscode_erlfmt_scan:get_inner_end_line(Meta) + 1 < vscode_erlfmt_scan:get_line(Comment) of
...
comment_to_algebra({comment, _Meta, Lines}) ->
    LinesD = lists:map(fun vscode_erlfmt_algebra:string/1, Lines),
    fold_doc(fun vscode_erlfmt_algebra:line/2, LinesD).

comments_with_pre_dot(Meta) ->
    {
        vscode_erlfmt_scan:get_anno(pre_comments, Meta, []),
        vscode_erlfmt_scan:get_anno(pre_dot_comments, Meta, []),
        vscode_erlfmt_scan:get_anno(post_comments, Meta, [])
    }.

comments(Meta) ->
    [] = vscode_erlfmt_scan:get_anno(pre_dot_comments, Meta, []),
    {vscode_erlfmt_scan:get_anno(pre_comments, Meta, []), vscode_erlfmt_scan:get_anno(post_comments, Meta, [])}.
