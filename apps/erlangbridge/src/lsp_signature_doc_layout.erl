...
%% @doc return a signature compatible to vscode model
...
are_function_equal(FName, Arity, E) ->
    ...
    Result = N == FName andalso length(A) >0 andalso (Arity == list_to_integer(A) orelse (Arity == any)),
    Result.
...
get_text(Name, Es) ->
    ...
    _Other ->
        lsp_log:warning(<<"LSP">>, "~p:get_text unknown xml content : ~p~n  ", [?MODULE, _Other]), 
        ""
    end.
...
signatures_sample() ->
    [
        #{
            label => <<"signature 1 from signature_doc_layout">>,
            documentation => #{
                kind => <<"markdown">>,
                value => <<"">>
            },
        ...
        #{
            label => <<"signature 2 from signature_doc_layout">>,
            documentation => #{
                kind => <<"markdown">>,
                value => <<"# Header \n Some text \n ```typescript\nsomecode();\n```">>
            },
